...
const app = {
    config: {
        ...
        dialogDepth: 0,
        contextStack: [],
        userFacts: storage?.data?.userFacts || {},
        sessionMemory: [],
        moodHistory: [],
        silenceCount: 0,
    ...
    // Упрощенный но функциональный лексикон (1500+ слов)
    lexicon: {
    ...
    // Исправленные сценарии (без функций внутри массивов, которые вызывали ошибки)
    scenarios: {
        askName: {
            phrases: [
        ...
        support: {
            triggers: ['грустно', 'плохо', 'устал', 'достало', 'тоска', 'плакать', 'больно', 'обидно'],
            responses: [
        ...
        food: {
            triggers: ['хочу есть', 'голоден', 'пицца', 'бургер', 'суши', 'кушать', 'готовить'],
            questions: [
        ...
        debate: {
            triggers: ['не согласен', 'спорим', 'по-другому', 'неправ', 'фигня', 'бред'],
            responses: [
        ...
        joke: {
            triggers: ['анекдот', 'шутка', 'пошути', 'смешно'],
            jokes: [
    ...
    init: function() {
        console.log('🧠 Глот v10.1 запущен');
    ...
    generateResponse: function(text) {
        ...
        if (state.expecting === 'name') {
            ...
            if (result.status === 'provided') {
                ...
                this.setEmotion('happy');
                return resp.replace('{name}', result.value);
            }
    ...
    parseNameResponse: function(text) {
        ...
        for (let pattern of patterns) {
            ...
            if (match && match[1]) {
                const notNames = ['не', 'да', 'нет', 'потом', 'может', 'думаю'];
        ...
        // Одно слово с заглавной
        if (text.trim().split(' ').length === 1 && text[0] === text[0].toUpperCase() && text.length > 2 && text.length < 15) {
            return { status: 'provided', value: text.trim() };
        }
    ...
    setPersonality: function(p) {
        ...
        if (label) label.textContent = 'Режим: ' + (names[p] || 'Друг');
        
        this.updateAvatar();
        this.speak(`Теперь я в режиме ${names[p]}!`);
    },
